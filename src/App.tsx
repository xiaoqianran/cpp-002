      ...
      <div className="mx-auto flex max-w-[1400px] flex-col gap-5 px-4 py-5 md:px-6 md:py-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside className="space-y-4">
            ...
            <Panel title="直接光照采样" icon={<Sparkles className="size-4" />}>
              <button
                type="button"
                data-testid="toggle-nee"
                onClick={() => setUseNee((v) => !v)}
                className={`flex h-11 w-full items-center justify-between rounded-[var(--radius-md)] border px-3 text-sm transition ${
                  useNee
                    ? "border-border-strong bg-bg-subtle"
                    : "border-border bg-bg text-fg-muted"
              ...
              >
                <span>NEE 面光重要性采样</span>
                <span className="font-mono text-xs">{useNee ? "ON" : "OFF"}</span>
              </button>
              <p className="text-xs text-fg-subtle">
